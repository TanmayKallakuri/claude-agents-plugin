...
from agent_tree import cmd_fail, TreeStore, parse_frontmatter

class TestCmdFail(unittest.TestCase):
    def setUp(self):
    ...
    def test_sets_status_failed(self, mock_find):
        mock_find.return_value = self.agents_dir
        args = type("Args", (), {"id": "task-1", "reason": "API unavailable"})()
        cmd_fail(args)
        store = TreeStore(self.agents_dir)
        data = store.load()
        self.assertEqual(data["agents"]["task-1"]["status"], "failed")

    @patch("agent_tree.TreeStore.find_agents_dir")
    def test_appends_reason_to_log(self, mock_find):
        mock_find.return_value = self.agents_dir
        args = type("Args", (), {"id": "task-1", "reason": "Dependency missing"})()
        cmd_fail(args)
        with open(os.path.join(self.agents_dir, "task-1.md")) as f:
            content = f.read()
        self.assertIn("Dependency missing", content)
        self.assertIn("FAILED", content)

    @patch("agent_tree.TreeStore.find_agents_dir")
    def test_md_frontmatter_status_failed(self, mock_find):
        mock_find.return_value = self.agents_dir
        args = type("Args", (), {"id": "task-1", "reason": "Broke"})()
        cmd_fail(args)
        with open(os.path.join(self.agents_dir, "task-1.md")) as f:
